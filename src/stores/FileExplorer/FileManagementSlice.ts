...
const createFEFileManagementSlice: StateCreator<IFEFileManagementSlice> = (
  set,
  get
) => ({
  loading: false,
  ...
  getDirectoryContents: async (path?: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/list?path=${encodeURIComponent(targetPath)}`,
        {
          method: "GET",
          headers: {
      ...
      toast.error(
        error.message || "An error occurred while fetching directory contents"
      );
      console.error(error);
      set({
        error: true,
